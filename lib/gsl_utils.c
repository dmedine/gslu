...
void gslu_mult_matrix_vector(gsl_matrix *x, double *vec){
...
void gslu_trans_scale(gsl_matrix *x, double *translate, double *scaler){
  ...
  if(x->size1 != 4|| x->size2 != 4)
    {
      printf("gsl_utils error: gslu_trans_scale:\n matrix is incorrect size\n");
      return;
    }
  ...
  for(i = 0; i<3; i++)
    gsl_matrix_set(x, i, i, scaler[i]);
  
  gslu_translate(x, translate[0], translate[1], translate[2]);
...
void gslu_viewport_transform(gsl_matrix *w, 
  ...
  float scale_y = height / window_height;
   ...
   double trans[] = {(double) offset_x, (double)offset_y, 0.0};
  double scale[] = {(double)scale_x, (double)scale_y, 1.0};

  gslu_trans_scale(w, trans, scale);
